...
test('user pastes in youtube url for a youtube already in the db with captions and he sees the captions load in from the db', async ({ page }) => {
  ...
  const has_captions = '9ruqSX_p_48'
  await page.getByPlaceholder(youtube_prefix).fill(youtube_prefix + has_captions)
  await expect(page.getByText('This is a fake transcript...')).toBeVisible()
})
...
// Test: anonymous user (not authed) in a separate tab, goes to url from previous test and sees the same video with the same captions
...
// Test: user can remove video from his videos
