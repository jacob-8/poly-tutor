...
import { auto_calculate_chapters } from './auto-calculate-chapters'

export async function get_chapters({youtube_id, duration_seconds}: { youtube_id: string, duration_seconds: number}): Promise<YoutubeChapter[]> {
  if (!CORS_PROXY_URL)
  ...
  try {
    ...
    return chapters || auto_calculate_chapters(duration_seconds)
  } catch (err) {
...
function extract_chapters_from_youtube_initial_data(youtube_initial_data: YouTubeInitialData): YoutubeChapter[] | null {
  ...
  if (!chapters_engagement_panel) return null
